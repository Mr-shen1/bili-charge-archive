...
- 更新日期：2026-09-27
- 当前目标：[开发实施计划](IMPLEMENTATION_PLAN.md) M6 已完成；真实私有 OSS、签名和页面图片的验收证据见 [M6 验收记录](ACCEPTANCE_REPORT_M6.md)。此前验收见 [M5 验收记录](ACCEPTANCE_REPORT_M5.md)、[M0～M2 回归记录](ACCEPTANCE_REPORT_M0_M2.md) 和 [M4 验收记录](ACCEPTANCE_REPORT_M4.md)
...
- 新增 `/api/ups`、`/api/dynamics`、动态详情、根评论和楼中楼查询。只读接口复用管理员会话鉴权；动态和根评论按发布时间、数字字符串 ID 倒序，楼中楼正序；固定 20 条、不透明且绑定查询范围的 cursor 支持前后页。动态卡片的评论数查询本站 `comment` 表，停用 UP 和来源不可用的内容仍返回；图片只返回状态与本站媒体路径，不返回来源图直链。
- Vue 增加 `/dynamics` 和 `/dynamics/{id}` 手机阅读页，提供 UP 筛选、独立详情、评论直接展示、楼中楼展开和分页。返回列表保留 URL 中的筛选及 cursor，并恢复滚动位置。未就绪图片显示占位；READY 图片使用本站媒体路径，访问失败时显示占位。Nginx 的 SPA 回退支持详情 URL 直接打开和刷新。
- JDK 21 下 `backend/mvnw.cmd -q test` 共 22 项通过，失败和错误均为 0；新增的真实 MySQL HTTP 测试覆盖 21 条同时间动态、21 条根评论与 21 条楼中楼、前后页、跨筛选 cursor 拒绝、本站评论数、图片元数据、停用 UP 和来源不可用。`frontend/npm run build` 通过；Spring 与 Nginx 镜像重建后本地运行。
- 在 390×844 浏览器视口核对单列布局；用独立的本地样例 UP 验证动态、根评论及楼中楼各 20+1 分页，停用与来源不可用提示、本站评论 42 条、返回筛选和列表位置、详情 URL 刷新。实际已存动态也能进入详情、直接查看根评论并展开楼中楼。浏览器验证错误密码被拒绝、正确密码后恢复详情、退出登录后详情要求重新登录；既有 `AuthHttpTest` 验证未登录读接口拒绝与无 CSRF 写请求拒绝。样例 UP、21 条动态及 42 条评论已从开发库清理，真实 UP 的 19 条动态保持不变。
- `UI-07` 中来源不可用的已存文字与图片状态已验证；按用户确认的 M5/M6 边界，READY 图片的实际内容由 M6 的 `/api/media` 签名跳转接口提供并在 M6 验证。未进入 M6，也未上传 OSS 或发送飞书。

## M6 实现与验证

- 新增 Spring 图片后台任务：从 `dynamic_image`、`comment_image` 认领待处理行，在事务外从限定的 B 站图片 CDN 下载并上传私有 OSS，成功后才写入 READY 与对象 Key。V3 迁移记录尝试次数，用来源 URL 和次数保护处理中的状态更新；暂时失败进入退避重试，明确的来源 410 或非法地址标为 UNAVAILABLE。图片故障不阻塞文字与评论查询。头像仍使用来源地址。
- `/api/media/dynamics/{dynamicId}/{position}` 和 `/api/media/comments/{dynamicId}/{rpid}/{position}` 复用登录会话鉴权，对 READY 图片返回 10 分钟 OSS V4 签名的 HTTPS 302；响应禁缓存和 Referer，未就绪为 409、确定不可用为 410。私有 Bucket 与对象 ACL 均经检查；OSS 未配置时后台任务不启动。
- Vue 在待处理图片状态下定时刷新已打开的内容，图片成功后换成实际图；READY 图片加载失败时保留占位并重试。浏览器中实际动态列表的媒体图片 `naturalWidth`、`naturalHeight` 均大于 0，详情页已存文字与评论仍可读。
- JDK 21 下带真实测试 Bucket 配置运行完整后端回归，27 项通过、失败/错误/跳过均为 0。`frontend/npm run build` 通过，本地 Compose 的 MySQL、Spring、Nginx 均为 healthy。真实运行、签名到期及隔离恢复细节见 [M6 验收记录](ACCEPTANCE_REPORT_M6.md)。没有实施飞书发送。
